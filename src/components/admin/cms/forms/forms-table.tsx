...
type ListItem = {
  ...
  kind: "ADMISSION" | "CONSENT" | "PRE_VISIT" | "INSURANCE" | "OTHER" | null;
  ...
  _count: { tags: number; categories: number; assets: number };
};
...
export default function FormsTable() {
  ...
  const [kind, setKind] = useState<"" | NonNullable<ListItem["kind"]>>("");
  ...
  async function load(page = 1) {
    ...
    try {
      const res = await fetch(`/api/admin/cms/forms?${params.toString()}`, {
        cache: "no-store",
      });
      if (!res.ok) throw new Error("خطا در دریافت لیست فرم‌ها/فایل‌ها");
  ...
  async function setStatusFor(id: string, next: ListItem["status"]) {
    const res = await fetch(`/api/admin/cms/forms/${id}`, {
      method: "PATCH",
  ...
  async function onDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/cms/forms/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
    ...
    >
      <div className="mb-7 ">
        ...
        <p className="text-sm text-gray-500 mt-1">
          آپلود PDF/تصویر/سند و انتشار برای دانلود.
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="جستجو بر اساس عنوان/اسلاگ/توضیح…"
            value={q}
          ...
          <input
            className="w-36 rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-navbar-secondary text-right"
            placeholder="زبان (fa/en)"
            value={language}
        ...
        <table className="w-full text-sm">
          <thead className="bg-navbar-primary">
            <tr className="text-right text-white">
              <th className="p-3 text-right w-20">پیش‌نمایش</th>
              <th className="p-3 text-right">عنوان / slug</th>
              ...
              <th className="p-3 text-right w-40">اقدامات</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              data.items.map((f) => (
                <tr key={f.id} className="border-t">
                  <td className="p-2 align-middle">
                    {f.previewImage?.publicUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={f.previewImage.publicUrl ?? ""}
                        alt={f.previewImage.alt ?? f.title}
                        className="w-14 h-14 object-cover rounded-lg border"
                      />
                    ) : (
                      <div className="w-14 h-14 rounded-lg bg-gray-100 border grid place-items-center text-[10px] text-gray-500">
                        {f.primaryFile?.mimeType?.includes("pdf") ? "PDF" : "—"}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="p-3 align-middle">{f.kind ?? "—"}</td>
                  <td className="p-3 align-middle">{f.language ?? "—"}</td>
                  <td className="p-3 align-middle">
                    <StatusBadge status={f.status} />
                  </td>
                  <td className="p-3 align-middle">{f.author?.name ?? "—"}</td>
                  <td className="p-3 text-xs font-semibold align-middle text-gray-600">
                    {new Date(f.updatedAt).toLocaleDateString("fa-IR")}
                  </td>
                  <td className="p-3 text-xs font-semibold align-middle text-gray-600">
                    {f.publishedAt
                      ? new Date(f.publishedAt).toLocaleDateString("fa-IR")
                      : "—"}
                  </td>
                  <td className="p-3 align-middle">
                    <div className="flex items-center gap-2 justify-end">
                      <Link
                        href={`/admin/cms/forms/${f.id}`}
                      ...
                      {f.status !== "PUBLISHED" ? (
                        <button
                          onClick={() => setStatusFor(f.id, "PUBLISHED")}
                        ...
                        <button
                          onClick={() => setStatusFor(f.id, "DRAFT")}
