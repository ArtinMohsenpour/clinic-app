...
export default function NewsTable() {
  ...
  async function load(page = 1) {
    ...
    try {
      const res = await fetch(`/api/admin/cms/news?${params.toString()}`, {
        cache: "no-store",
      });
      if (!res.ok) throw new Error("خطا در دریافت لیست اخبار");
  ...
  async function onDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/cms/news/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
  ...
  return (
    ...
    >
      <div className="mb-7">
        <h1 className="text-3xl font-extrabold text-navbar-primary">اخبار</h1>
        <p className="text-sm text-gray-500 mt-1">
          مدیریت، جستجو و به‌روزرسانی وضعیت انتشار اخبار.
        </p>
      </div>
                ...
                <tr key={n.id} className="border-t">
                  <td className="p-2 align-middle">
                    {n.cover?.publicUrl ? (
                      ...
                      <img
                        src={n.cover.publicUrl}
                        alt={n.cover.alt ?? n.title}
                  ...
                  <td className="p-3 text-xs font-semibold align-middle text-gray-600">
                    {n.publishedAt
                      ? new Date(n.publishedAt).toLocaleDateString("fa-IR")
                      : "—"}
                  </td>
                  ...
                  <td className="p-3 align-middle">
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      {n.status !== "PUBLISHED" ? (
                        <button
                          onClick={() => setStatusFor(n.id, "PUBLISHED")}
                        ...
                        <button
                          onClick={() => setStatusFor(n.id, "DRAFT")}
