...
export default function ArticleTable() {
  ...
  async function load(page = 1) {
    ...
    try {
      const res = await fetch(`/api/admin/cms/articles?${params.toString()}`, {
        cache: "no-store",
      });
      if (!res.ok) throw new Error("خطا در دریافت لیست مقالات");
  ...
  async function onDelete(id: string) {
    ...
    // if last item on page removed, step back a page (when possible)
    const nextPage =
  ...
  return (
    ...
    >
      <div className="mb-7 ">
        <h1 className="text-3xl font-extrabold text-navbar-primary">مقالات</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-navbar-primary">
            <tr className="text-right text-white">
              <th className="p-3 text-right w-20">کاور</th>
